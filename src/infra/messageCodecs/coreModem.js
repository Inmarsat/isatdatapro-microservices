...
function parseCoreModem(message) {
  if (!message.payloadJson || message.codecServiceId !== 0) {
    throw new Error(`Attempt to parse message ${message.messageId}`
        + ` as core modem`);
  }
  ...
  switch (messageType) {
    ...
    case 98:
      return parseModemLastRxInfo(message);
      //break;
    case 99:
      return parseModemRxMetrics(message);
      //break;
    case 100:
      return parseModemTxMetrics(message);
      //break;
    case 112:
      return parseModemPingReply(message);
      //break;
    case 113:
      return parseNetworkPingRequest(message);
      //break;
    case 115:
...
/**
 * Parses location and timestamp data to update the IdpMobiles
 * collection with device metadata
 * @private
 ...
 * @param {(string|number)} periodCode The period over which metrics were
 *    calculated by the modem
...
function pingTime(timestamp) {
  ...
  const pingTime = ((d.getUTCHours() * 3600
      + d.getUTCMinutes() * 60
      + d.getUTCSeconds())
      % 65535);
  return pingTime;
...
/**
 * Parses request from modem for network ping response
 * (note: response is automatically generated by the network)
 * @private
 * @param {MessageReturn} message The return message
 */
...
function commandSetMobileWakeupPeriod(mobileWakeupPeriod) {
  ...
  if (typeof(mobileWakeupPeriod) === 'string') {
    ...
    for (let prop in wakeupPeriods) {
      if (wakeupPeriods.hasOwnProperty(prop)
          && wakeupPeriods[prop] === mobileWakeupPeriod) {
        isValid = true;
        break;
      }
    }
